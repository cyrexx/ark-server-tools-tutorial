...
### 12.4 LinuxGame.ini and GameUserSettings.ini ###
...
`vim /home/ARK_SERVER_USERNAME/ARK/ShooterGame/Config/Linux/LinuxGame.ini` :: Path to (edit) _LinuxGame.ini_ (change **ARK_SERVER_USERNAME**).
`vim /home/ARK_SERVER_USERNAME/ARK/ShooterGame/Saved/Config/LinuxServer/GameUserSettings.ini` :: Path to (edit) _GameUserSettings.ini_ (change **ARK_SERVER_USERNAME**).
...
```
`arkmanager installmods` :: Install the Mod(s) via arkmanager.
`arkmanager installmods @all` :: Install the Mod(s) via arkmanager, if server directories are seperate (not applicable for this tutorial).
